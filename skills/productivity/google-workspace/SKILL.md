...
- `references/calendar-live-notifications.md` — Push event notifications via polling watcher + cron job (no_agent=True). Python script checks Google Calendar every 10 min, outputs notification text, deduplicates via JSON tracker.
...
- **Anything beyond email** → Continue with this skill. The setup script always
  requests all available scopes (calendar + gmail + drive + sheets + docs +
  contacts), but the user can **deselect individual permissions** in Google's
  consent screen before approving. Let them know they can uncheck services they
  don't need.
...
Run setup.py to generate the URL (it always requests all scopes):

```bash
$GSETUP --auth-url
```

This prints the OAuth URL directly as plain text.
