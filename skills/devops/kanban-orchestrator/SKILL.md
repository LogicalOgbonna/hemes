...
### Iteration budget exhaustion recovery

A different class of stuck worker: **the worker completed work but hit the agent iteration budget (default 90 tool calls) before it could finish.** The task ends up `blocked` with reason "Iteration budget exhausted." Unlike the crash/hallucination case above, the worker likely produced valuable partial work. Recovery is about saving that work and continuing, not restarting.

Pattern when a worker exhausts its budget:

1. **Read the worker's last heartbeat and comments** via `kanban_show(task_id)`. The heartbeat usually says what was accomplished ("scaffolded X, created DB Y, waiting for Z"). Comments may contain the worker's progress notes.

2. **Inspect the scratch workspace** at the path in `kanban_show()`'s `workspace_path` field. Scratch workspaces are **ephemeral** — they remain until the task is completed or removed, but there's no guarantee they'll survive. Copy any valuable code out immediately:
   ```bash
   cp -r /path/to/workspace /durable/location/
   ```
   For a git repo, also check if the worker committed any changes with `git log`.

3. **Save the work to durable storage** — commit to the project repo, or copy to a monitored directory. Remove any `.env` or credential files before committing:
   ```bash
   rm workspace/.env 2>/dev/null
   git add workspace/
   git commit -m "feat: partial work from worker [task-id]"
   ```

4. **Close the original task** as done (partial) with metadata recording what was accomplished and what remains:
   ```python
   kanban_complete(
       task_id="t_xxxx",
       summary="Worker built X but hit budget before Y. Code saved to repo.",
       metadata={
           "scaffolded": True,
           "neon_db_created": "project-name",
           "committed_to": "path/in/repo",
           "remaining": ["api_routes", "ui", "vercel_deploy"],
       },
   )
   ```

5. **Create a focused follow-up task** for the remaining work. The new task should be scoped to what was left — NOT a re-run of the original. Explicitly tell the worker the foundation is done and what to skip:
   ```
   Title: "Finish X — remaining pieces after budget hit"
   Body:
   ## Where to work
   Code is at /absolute/path/to/project. The DB is already created at URL.
   Do NOT create a new project/database — reuse the existing one.
   Do NOT re-scaffold or restructure.

   ## Acceptance Criteria
   - [ ] Task 1 (what was left)
   - [ ] Task 2
   ```

6. **Link the follow-up as a child** of the original task so the dependency chain is visible on the board:
   ```python
   kanban_link(parent_id="t_original", child_id="t_followup")
   ```

**Key why this matters:** Without this pattern, scratch workers that hit budget silently lose all their work — the workspace is deleted when the task is garbage-collected, weeks or months later. The orchestrator is the only one who can save it.

### Task scope estimation

The default agent iteration budget is 90 tool calls. When creating tasks, estimate whether they'll fit:

- **Fits in 90:** Single-focus task (add one API endpoint, write tests for one feature, fix one bug, deploy an existing project). These have well-defined input and output — the worker reads a spec and writes code, tests, deploys.
- **Too large for 90:** Multi-phase task that requires both scaffold-and-configure AND implement-and-deploy. Example: "Build a full-stack todo app" involves (a) scaffold project, (b) create DB, (c) set up ORM, (d) build CRUD API, (e) build UI, (f) quality gate, (g) deploy, (h) smoke-check. That's ~8 distinct phases, each consuming 5-15 tool calls.
- **Good rule of thumb:** if the task body has more than 4 top-level acceptance criteria, consider splitting. A single large task can be split into:
  - **Phase 1:** Scaffold + infrastructure (project setup, DB creation, ORM, config) → typically 30-40 iterations
  - **Phase 2:** Implementation + deploy (write code, quality gate, deploy, smoke-check) → typically 40-60 iterations
